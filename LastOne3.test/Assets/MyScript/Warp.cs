...
public class Warp : MonoBehaviour {
	...
	void OnTriggerEnter(Collider col){
		if (col.tag == "Character") {
			...
			else if (Mode == 2 && data.NowMode == 3) {
				Vector3 tmp = new Vector3 (col.transform.position.x, col.transform.position.y, box.transform.position.z);
				col.transform.position = tmp;
			}
		}
	}
