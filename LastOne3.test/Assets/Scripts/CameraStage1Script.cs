...
public class CameraStage1Script : MonoBehaviour {
	...
	void Update () {
		...
		if(mode == 2){
			MyCam [2].transform.position = new Vector3 (PlayerPosition.transform.position.x + 7, PlayerPosition.transform.position.y + 7, PlayerPosition.transform.position.z - 7);
		}
		if (mode == 4) {
			MyCam [0].SetActive (false);
			MyCam [1].SetActive (true);
